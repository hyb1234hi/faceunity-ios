...
@property (nonatomic, assign) CVPixelBufferRef jianHuang;

@property(nonatomic, assign) BOOL fuRendererInited;
...
- (void)initFURenderer:(NSString *)a {
    if (self.fuRendererInited) {
        return;
    }
    ...
    NSData *d = [[NSData alloc] initWithBase64EncodedString:a options:NSDataBase64DecodingIgnoreUnknownCharacters];
    int authSize = (int) [d length];
    char *auth = (char *) [d bytes];
    int retCode = [[FURenderer shareRenderer] setupWithData:nil dataSize:0 ardata:nil authPackage:auth authSize:authSize shouldCreateContext:YES];
    NSLog(@"FURenderer setup, retCode: %d authSize:%d %d %d", retCode, authSize, auth[0], auth[authSize - 1]);
    self.fuRendererInited = true;
}
...
-(void)loadAIModle{
    ...
    NSData *ai_human_processor = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"ai_human_processor" ofType:@"bundle"]];
    ...
    NSData *ai_face_processor = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"ai_face_processor" ofType:@"bundle"]];
...
- (void)setBeautyParameters{
       for (FUBeautyParam *modle in _skinParams){
        ...
        NSLog(@"setBeautyParameters param:%@ value: %f", modle.mParam, modle.mValue);
    }
    ...
    if (self.seletedFliter) {
        ...
        NSLog(@"setBeautyParameters filter_name:%@ filter_level: %f", self.seletedFliter.mParam, self.seletedFliter.mValue);
    }
}
...
-(void)setupSkinData{
    ...
    NSDictionary *defaultValueDic = @{@"blur_level":@(1.0),@"color_level":@(0.3),@"red_level":@(0.3),@"sharpen":@(0.2),@"remove_pouch_strength":@(0),@"remove_nasolabial_folds_strength":@(0),@"eye_bright":@(0),@"tooth_whiten":@(0)};
...
/**销毁全部道具*/
- (void)destoryItems {
    NSLog(@"FUManager namaLibDestroy");
    [FURenderer destroyAllItems];
    [FURenderer namaLibDestroy];
    self.fuRendererInited = false;
...
- (CVPixelBufferRef)renderItemsToPixelBuffer11{
    return self.jianHuang;
}
